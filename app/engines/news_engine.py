...
def score_article(article: RawNewsArticle, symbol: str) -> ScoredArticle:
    ...
    if abs(score) < 0.06:
        ...
        impact = "bearish"
    # Reliability and market relevance are separate. An official source is trustworthy,
    # but an unrelated administrative announcement is not market-moving.
    importance = "high" if high_matches and relevance > 0 else "medium" if relevance >= 0.67 else "low"
...
def analyze_news(articles: Iterable[RawNewsArticle], symbol: str, use_ollama: bool | None = None) -> NewsAnalysis:
    ...
    # Never count an article as relevant merely because it came from a central bank.
    relevant = [item for item in scored if item.relevance >= 0.34]
